...
/* ------------------------------------------------------------------------------ */
global $wpdb, $awsomcreatecachedir, $awsomcreatedefaultgallerydir, $awsom_pixgallery_db_version;
...
$comments_skip[0]->comment_content = "No Comments Posted Yet For This Image";
$comments_skip[0]->comment_author = "AWSOM Pixgallery Administrator";
}
elseif (empty($comments_skip) && $PixgalleryDisplayingImage == "gallery"){
$comments_skip[0]->comment_content = "No Comments Posted Yet For This Gallery";
$comments_skip[0]->comment_author = "AWSOM Pixgallery Administrator";
}
...
function PixGallery_Image($Html)
{
	...
	global $WebsiteRoot, $WebsitePath, $WebsiteActualPath, $PixGallery_UseAbsPath, $PixGallery_CustomPopup, $PixGallery_CustomRel;
...
function PixGallery_PhotoGallery_Collection()
{
	...
	global $PixGallery_Rows, $PixGlobal_Path;
...
//**Mod for AWSOM News Link and Remove _ from file name addition//
	global $PixGallery_LinkToNewsPost, $PixGallery_RemoveUnderscoresFromName, $PixGallery_ImageCaption, $PixGallery_HideSidebar, $PixGallery_MaxThumbnailsOnIndex, $wpdb, $PixGallery_CustomCSS, $PixGallery_LegacyMode, $post;
	global $awsom_pixgallery_caption_table_name, $awsom_pixgallery_gallery_table_name, $PixGallery_OpenInNewWindow, $customdirimage, $PixGallery_NoThumbnailText, $PixGallery_WatermarkImages, $PixGallery_ExtraNavBars;
	global $PixgalleryDisplayingImage, $Pixgalleryitempath, $PixgalleryReturnPath, $PixGallery_Displayexif, $PixGallery_DebugMode, $PixGallery_LimitPaginationLinks, $PixGallery_CustomPopup, $PixGallery_CustomRel, $PixGallery_Displayiptc;
//**end Mod //
...
//****End AWSOM mod****//
	...
	// *****AWSOM mod to get Gallery Custom name if exist ******//
		...
		$Output	.= "<a href='$ContentOverallParent'>$ParentName</a> &rsaquo; ";
		...
		for ($i = 0; $i != count($PathParts); $i += 1)
			...
			//******* AWSOM mod to add Custom Image Name in Breadcrumb *******//
			$customnamepathmatch = $GalleryPath.$CollectionPath;
			$customnamepathmatch = str_replace("//", "/", $customnamepathmatch);
			//$pathpartsname = $PathParts[$i];
			//$Output .= $Link.$PathParts[$i];
			//********CAN I ADD SUBGALLERY CUSTOMNAMES TO THIS FUNCTION?*********//
			...
			$ShowCustomSubGalNameInBreadcrumb = $wpdb->get_var("SELECT galcustomname FROM $awsom_pixgallery_gallery_table_name WHERE galpath = '$customnamepathmatch'");
			}
			...
			//$Output .= "$i $ShowCustomImageNameInBreadcrumb $customnamepathmatch $PathParts[$i]";
			}
			
			
			//if ($ShowCustomSubGalNameInBreadcrumb !="" ) {
			//$ShowCustomSubGalNameInBreadcrumb = stripslashes($ShowCustomSubGalNameInBreadcrumb);
			//$Output .=  "<a href='$Link'>".ucwords(strtolower($ShowCustomSubGalNameInBreadcrumb))."</a>";
			//$Output .= "$i $ShowCustomImageNameInBreadcrumb $customnamepathmatch $PathParts[$i]";
			//}
			
			else {
			$Output .=  "<a href='$Link'>".ucwords(strtolower($PathParts[$i]))."</a>";
			}
	...
	if (($CollectionPath != "/") && ($ContentParentUrl != ""))
		...
		if (is_file($FullPath))
			...
			if (isset($PreviousLink))
				$Output .= " <center><a href='".$PreviousLink."'>Previous Image</a> ";
			else
				$Output .= " <center>Previous Image ";

			$ThisImageInCount = $ContentList[0]['ImageCountNumber'];
			$Output .= ": Image $ThisImageInCount :";
			if (isset($NextLink)){
			//*****AWSOM MOD ADDITION******
				$Output .= " <a href='".$NextLink."'>Next Image</a>";
				}
				else {
				$Output .= " Next Image";
				}
			...
			if (isset($PreviousLink) && is_dir($PreviousPath))
				$Output .= "<center> <a href='".$PreviousLink."'>Previous Archive</a>";
			if (isset($NextLink) && is_dir($NextPath)){
					if (isset($PreviousLink)){
					$Output .= " :: <a href='".$NextLink."'>Next Archive</a>";
					}
					else {
					$Output .= "<center> <a href='".$NextLink."'>Next Archive</a>";
					}
				}
			$Output .= "</center>";
			}
...
//*** AWSOM Image view tracking mod ***//
		...
		$ShowCurrentImageViewCount = "<br />Image View Count: ";
		$ShowCurrentImageViewCount .= $wpdb->get_var("SELECT viewnum FROM $awsom_pixgallery_caption_table_name WHERE imagepath = '$addimagepathincaption'");
			}
...
//**Mod for AWSOM News Link  //
		...
		if ($newspostlinks) {
		$newslink .= "<br /><b>News Posts for this Day:</b><br />";
		...
		$pxgexifdataoutput = "EXIF Data<br /><p style='text-align:left;'>";
...
$pxgiptcdataoutput .= "IPTC Data<br /><p style='text-align:left;'>";
...
$pxgiptcdataoutput.= "</p>";
		...
		if ($ContentIsFile == TRUE && $PixGallery_OpenInNewWindow == "1")
			...
			$Output .= "<td><center><a href=\"$ImagePath\" title=\"$ImageAlt\" rel='$PixGalleryUseRel' target=\"_blank\"><img border=0 src=\"$ImagePath\" width='$ImageWidth' class='PxgImage' alt=\"$ImageAlt\"></a><p>$titleforimage</center> $captionforimage <center>$newslink $ShowCurrentImageViewCount $pxgexifdataoutput $pxgiptcdataoutput</p></center>";
		...
		$Output .="</td>\n";
			...
			//** end bottom nav bar display area **//
				...
				if ( function_exists('current_user_can') && current_user_can('manage_options') ) {
								...
								$PxgImageEditID = $wpdb->get_var("SELECT capid FROM $awsom_pixgallery_caption_table_name WHERE imagepath = '$addimagepathincaption'");
									...
									$Output .="<form method=\"post\" action=\"$wp_pxgadmin_url/wp-admin/post-new.php?page=pixgallery.php\">";
									...
									$Output .="<tr><td><input type=\"submit\" value=\"Add Image Caption\" name=\"admin_edit_link_on_page\"></form></td></tr>";
									}
									else {
									$Output .="<form method=\"post\" action=\"$wp_pxgadmin_url/wp-admin/post-new.php?page=pixgallery.php\">";
									...
									$Output .="<tr><td><input type=\"submit\" value=\"Edit Image Caption\" name=\"caption_update_edit\"></form></td></tr>";
									}
				}
			}
			...
			$titleforimage = "";
				...
				$Output .= "<td><center><a href=\"$ImagePath\" title=\"$ImageAlt\" rel='$PixGalleryUseRel'><img border=0 src=\"$ImagePath\" width='$ImageWidth' class='PxgImage' alt=\"$ImageAlt\"></a><p>$titleforimage</center> $captionforimage <center>$newslink $ShowCurrentImageViewCount $pxgexifdataoutput $pxgiptcdataoutput</p></center>";
				$PixgalleryDisplayingImage = "image";
				$Pixgalleryitempath = $ContentList[$i]['Imagesortpath'];
						//**Display bottom Nav bar if setting is turned on **//
						if ($PixGallery_ExtraNavBars == "1"){
		...
		$ImageUrl = get_bloginfo('wpurl').$ImageUrl;
			...
			$displaysortidposition = "(".$ContentSortNumber.")";
			}
			if ($PixGallery_NoThumbnailText !== "1"){
			if ($PopupOnThumbnails == TRUE && $ContentType != "Gallery"){
							if ($PixGallery_OpenInNewWindow == 1){
							$Output .= "<a href=\"$ImagePathpopup\" rel='$PixGalleryUseRel' title=\"$ImageAlt\" target=\"_blank\">$ContentTitle $displaysortidposition</a>";
							}
							else {
							$Output .= "<a href=\"$ImagePathpopup\" rel='$PixGalleryUseRel' title=\"$ImageAlt\">$ContentTitle $displaysortidposition</a>";
							}
						}


			else {
				$Output .= "<a href=\"$ImageUrl\">$ContentTitle $displaysortidposition</a>";
				}
				}
			else {
	...
	if (!is_page() && !is_single()){
					...
					$Output .= "<p align=\"center\"><a href=\"".$ContentParentUrl."\" rel=\"bookmark\">Click Here To View Entire Gallery</a></p>";
					}
			}
	//***** end AWSOM mod *****//
	if ($PixGallery_Rows && !is_home())
		...
		if ($TotalPages > 1)
			...
			if (($PrevStartImage >= 0) && ($PrevStartImage != $NextStartImage))
				{
				$Output .= "<a href='".PixGallery_UrlBuild($QueryStartName)."'>First</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href='".PixGallery_UrlBuild($QueryStartName, $PrevStartImage)."'>Previous</a>";
				}

			$Output .= "</td><td align=center>$CurrentPage / $TotalPages</td><td align=right>";

			if ($NextStartImage < $TotalImages)
				{
				$Output .= "<a href='".PixGallery_UrlBuild($QueryStartName, $NextStartImage)."'>Next</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href='".PixGallery_UrlBuild($QueryStartName, $LastPage)."'>Last</a>";
				}
			...
			for ($pi = 0; $pi != $TotalPages; $pi += 1){
						...
						$Output .= "<a href='".PixGallery_UrlBuild($QueryStartName, $PreviousLinkSpecial)."' class='PxgPaginationLink'>...</a>&nbsp;";
						}
						$Output .= "$PaginationLink";
						if ($highestpoint < $TotalPages){
						$NextLinkSpecial = $highestpoint * $ImagesPerPage;
						$Output .= "<a href='".PixGallery_UrlBuild($QueryStartName, $NextLinkSpecial)."' class='PxgPaginationLink'>...</a>";
						}
		  ...
		  // $Output .="</table>";
			...
			if ( function_exists('current_user_can') && current_user_can('manage_options') ) {
								...
								$Link = $ReturnLinkFromEdit;
									...
									$Output .="<tr><td><input type=\"submit\" value=\"Add Gallery Caption\" name=\"admin_edit_gallerylink_on_page\"></form></td></tr>";
									}
									else {
									$Output .="<form method=\"post\" action=\"$wp_pxgadmin_url/wp-admin/themes.php?page=pixgallery.php\">";
									...
									$Output .="<tr><td><input type=\"submit\" value=\"Edit Gallery Caption\" name=\"gallery_update_edit\"></form></td></tr>";
									}
	...
	$Output .= "</td></tr></table> </div><!-- End AWSOM Pixgallery Main Table -->";
	if ($Link == "" && $PaginationLink ==""){
									$Link = $ContentParentUrl;
									}
									elseif ($Link == "" && $PaginationLink !=""){
...
<blockquote>
	<table border=0 cellpadding=0 cellspacing=0>
		<tr>
			<td width="360">Maximum thumbnail page image size (0 = no max)</td>
			<td align=right>Width:<input type='text' name='PixGallery_ImageWidth' style='width: 40px;' value='<?php echo $GLOBALS["PixGallery_ImageWidth"]; ?>'> px &nbsp;&nbsp;Height:<input type='text' name='PixGallery_ImageHeight' style='width: 40px;' value='<?php echo $GLOBALS["PixGallery_ImageHeight"]; ?>'> px</td>
		</tr>
	</table>
	<br />
	<table border=0 cellpadding=0 cellspacing=0>
		<tr>
			<td width="360">Maximum image size for viewing full size images within the Wordpress page (use this setting if you need to scale full size images to fit your theme)(0 = no max)</td>
			<td align=right>Width:<input type='text' name='PixGallery_ImageWidthLarge' style='width: 40px;' value='<?php echo $GLOBALS["PixGallery_ImageWidthLarge"]; ?>'> px &nbsp;&nbsp;Height:<input type='text' name='PixGallery_ImageHeightLarge' style='width: 40px;' value='<?php echo $GLOBALS["PixGallery_ImageHeightLarge"]; ?>'> px</td>
		</tr>
	</table>
	<br />
	...
	<br /><br />
	If you often upload pictures straight from your digital camera, you can use this option to reduce the actual size of the images in Wordpress (Note: this is an actual file rewrite of your image file to make it smaller). Specify the image height/width you want your images to be reduced to. <b>Note:</b> requires 777 permissions on all directories that hold images including the wp-content/uploads area and all sub-folders.
	<br /><br />

	<!--- begin AWSOM mod section -->
	<hr>
	<h3>AWSOM PixGallery Mod 4.5.7 Options section.</h3> Please read the <a href="<?php echo $PixGlobal_AdminPath; ?>awsom_pixgallery_mod_helpfile.txt" target="_blank">Helpfile</a> for More information. For support or to check for newer versions visit <a href="http://www.awsom.org" target="_blank">AWSOM.org</a> or the support forum at <a href="http://www.harknell.com" target="_blank">Harknell.com</a>
	<br /><a href="http://www.awsom.org/tutorialsvideo/" target="_blank">Click Here</a> To Watch Video Tutorials or <a href="http://www.awsom.org/awsom-pixgallery/" target="_blank">Click Here</a> to View Online Documentation.<br /><br /> To add a gallery to your site use the following code in a post or a page:<b> &#60;pixgallery path="/<i>Your_image_directory_name</i>/"&#62;&#60;/pixgallery&#62;</b><br />
	Replace the "Your_image_directory_name" with your image folder starting from the root directory of your website. You can go multiple folders in, just make sure the last folder<br />
	...
	<!--- ****Mod added for Debug/Admin Mode***** -->
		...
		<br />
				...
				</select><a href="<?php echo $PixGlobal_AdminPath; ?>help_docs/debug_mode_setting.txt" style="text-decoration:none" target="_blank"><img src="<?php echo $PixGlobal_AdminPath; ?>images/help.png" border="0" align="top" title="Click here to get Help on the Debug Mode setting" alt="Get Help on the Debug Mode Setting"></a>
				<br /><br />
	<!--- *****end Debug Mode mod ***** -->
	...
	<br />
		...
		</select><a href="<?php echo $PixGlobal_AdminPath; ?>help_docs/news_interlink_setting.txt" style="text-decoration:none" target="_blank"><img src="<?php echo $PixGlobal_AdminPath; ?>images/help.png" border="0" align="top" title="Click here to get Help on News Interlink settings" alt="Get Help on the News Interlink Setting"></a>
		<br /><br />
	<!--- *****end News Link Mod ***** -->
	...
	If "No Javascript" is selected, do you want full size images to open in a new window instead of within the same window?
	<br />
		...
		if (isset($_POST['gallery_update_edit']))
			...
			<blockquote>
				<table border=0 cellpadding=0 cellspacing=0>
					...
					<tr>
					<tr>
						<td width=\"400\">Path To Gallery (from root folder of your Wordpress website--start with a / and end with a /):<br />
...
<blockquote>
	<table border=0 cellpadding=0 cellspacing=0>
		<tr>
			<td width="360">Image Name (*with* file extension):<br />
			<input type='text' value='<?php echo $editcaptionfrompage; ?>' name="newimagename" style="width: 150px;"></td></tr>
		<tr>
					<td width="360">Image Path (including file name):<br />
...
add_action('activate_awsompixgallery/pixgallery.php','PixGallery_CaptionTableInstall');
